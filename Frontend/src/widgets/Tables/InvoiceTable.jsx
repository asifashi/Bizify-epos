...
const InvoiceTable = ({setinvoiceId,setEdit,setSelectedInvoice,setView,setAcc,setAccview,okModalStock,okModal}) => {
  ...
  const  fetchInvoice= () =>{
    setLoading(true); 
    axios.get('http://localhost:8081/api/getinvoice', {
      headers: {
  ...
  const handleRemove = (VoucherNo) => {
    ...
    axios.delete(`http://localhost:8081/api/removeData/${VoucherNo}`)
      .then(response => {
  ...
  const handleAccounsClick = (VoucherNo)=>{
    setAccview(true)
    setAcc(VoucherNo)
  }
  const handleEdit = (VoucherNo)=>{
        setEdit(true);
        setinvoiceId(VoucherNo)
      ...
      const handleSearch = (e) => {
        ...
        if (invoice && invoice.length > 0) {
          const filteredData = invoice.filter((invoic) => {
            const propertyValue = invoic[selectedSearchOption];
      ...
      const paginatedInvoice = currentInvoice.slice(0, InvoicePerPage);
  ...
  <span className="font-medium">Info alert!</span> Invoice item deleted successfully
</Alert>
)}

{okModalStock && (
  <Alert color="success" icon={HiInformationCircle}>
    <span className="font-medium">Info alert!</span> Invoice item created successfully
  </Alert>
)}
{okModal && (
  <Alert color="success" icon={HiInformationCircle}>
    <span className="font-medium">Info alert!</span> Invoice item updated successfully
  </Alert>
)}
    ...
    <div className="text-center">
      ...
      <div className="flex justify-center gap-4">
        ...
        <div class="relative w-full mt-1">
            ...
            <input type="text" onChange={handleSearch} id="table-search" class="block outline-none p-2 pl-10 text-xs text-gray-900 border border-gray-300 rounded-r-md w-96 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for Invoice" />
          
        </div>
    </div>  )}
    {!loading && filteredInvoice.length === 0 && (
  <div className="text-center text-gray-500 dark:text-gray-400 mt-4">
    No search results found.
           ...
           <table className="w-full text-sm text-left text-gray-500 dark:text-gray-600">
              ...
              {Array.isArray(paginatedInvoice) && paginatedInvoice.map(invoice => (
             
                 <tr key={invoice.V_ID} className="bg-white border-b text-xs dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                   ...
                   <td  onClick={(e) => handleAccounsClick(invoice.VoucherNo)} className="px-4 py-3 cursor-pointer font-medium text-gray-900 whitespace-nowrap dark:text-white">
                    ...
                    <td className="px-4 py-3">
                    {new Date(invoice.VoucherDate).toLocaleDateString()}
                    </td>
                    <td className="px-4 py-3">
                    {new Date(invoice.DueDate).toLocaleDateString()}
                    </td>
                    <td className="flex items-center px-4 py-3 space-x-3">
                    <EyeIcon onClick={(e) => handleInvoiceClick(invoice.VoucherNo)} className='w-4 cursor-pointer h-4'/>
                    <PencilSquareIcon onClick={(e)=>handleEdit(invoice.VoucherNo)} className="w-4 h-4 text-blue-600 hover:cursor-pointer dark:text-blue-500 hover:underline"/>
                  <TrashIcon onClick={() => handleRemove(invoice.VoucherNo)} className="font-medium w-4 h-4 hover:cursor-pointer text-red-600 dark:text-red-500 hover:underline"/>
                </td>
    ...
    <div className='grid col-span-2'>
      <span className="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full ">
        Showing <span className="font-semibold text-gray-900 dark:text-white">
          {indexOfFirstInvoice + 1}-{indexOfLastInvoice > filteredInvoice.length ? filteredInvoice.length : indexOfLastInvoice}
        </span> of <span className="font-semibold text-gray-900 dark:text-white">{filteredInvoice.length}</span>
      </span>
    </div>
    <div className='grid col-span-2'>
      <ul className="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
        ...
        {Array.from({ length: Math.ceil(filteredInvoice.length / InvoicePerPage) }, (_, index) => (
          <li key={index}>
