...
const StockTable = ({setEdit,setstockId,setSelectedStock,setView,okModalStock,okModal}) => {
  ...
  const [showDeleteAlertStock, setShowDeleteAlertStock] = useState(false);
  const [selectedSearchOption, setSelectedSearchOption] = useState('ItemId');
  ...
  const fetchStock = () => {
    setLoading(true);
    axios.get('http://localhost:8081/api/stock', {
      headers: {
  ...
  const handleRemove = (ItemId) => {
    ...
    setConfirmationCallback(() => () => {
      axios
        .delete(`http://localhost:8081/api/stock/${ItemId}`)
        .then((response) => {
          console.log(response.data);
          setShowDeleteAlertStock(true);
          setTimeout(() => {
            setShowDeleteAlertStock(false);
          }, 2000);
          fetchStock(); // Fetch stocks again after deletion
        })
        .catch((error) => {
  ...
  const handleEdit = (ItemId)=>{
      ...
      const handleSearch = (e) => {
        ...
        if (stocks && stocks.length > 0) {
          const filteredData = stocks.filter((stock) => {
            const propertyValue = stock[selectedSearchOption];
      ...
      const handleStockClick = (ItemId) => {
        // Set the selected customer when a row is clicked
        setView(true)
        setSelectedStock(ItemId);
      };
      const indexOfLastStock = currentPage * StockPerPage;
      const indexOfFirstStock = indexOfLastStock - StockPerPage;
      const currentStock = filteredStocks.slice(indexOfFirstStock, indexOfLastStock);
      const paginatedStock = currentStock.slice(0, StockPerPage);
  ...
  return (
    <div className=' w-full justify-center'>
      {showDeleteAlertStock && (
  <Alert color="success" icon={HiInformationCircle}>
  <span className="font-medium">Info alert!</span> Stock item deleted successfully
</Alert>
)}

{okModalStock && (
  <Alert color="success" icon={HiInformationCircle}>
    <span className="font-medium">Info alert!</span> Stock item created successfully
  </Alert>
)}
{okModal && (
  <Alert color="success" icon={HiInformationCircle}>
    <span className="font-medium">Info alert!</span> Stock item updated successfully
  </Alert>
)}
...
{!loading && filteredStocks.length === 0 && (
  <div className="text-center text-gray-500 dark:text-gray-400 mt-4">
    No search results found.
           ...
           {!loading && filteredStocks.length > 0 && ( <table className="w-full text-sm text-left text-gray-500 dark:text-gray-600">
          ...
          <tbody>
            {Array.isArray(paginatedStock) && paginatedStock.map(stocks => (
              <tr key={stocks.ItemId} className="bg-white border-b text-xs dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td className="px-4 py-3">
                  {stocks.UnitPrice.toFixed(2)}
                </td>
                <td className="px-4 py-3">
                <img src={`data:image/jpeg;base64,${stocks.Image}`} alt={stocks.ItemName} />
                </td>
               
                <td className="flex items-center px-4 py-3 space-x-3">
                <EyeIcon  onClick={(e) => handleStockClick(stocks.ItemId)} className=' cursor-pointer w-4 h-4'/>
                  <PencilSquareIcon onClick={(e)=>handleEdit(stocks.ItemId)} className="font-medium w-4 h-4 text-blue-600 hover:cursor-pointer dark:text-blue-500 hover:underline"/>
                  <TrashIcon onClick={(e) => handleRemove(stocks.ItemId)} className="w-4 h-4 font-medium text-red-600 hover:cursor-pointer dark:text-red-500 hover:underline"/>
                </td>
              </tr>
            ))}
    ...
    <div className='grid col-span-2'>
      <span className="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full ">
        Showing <span className="font-semibold text-gray-900 dark:text-white">
          {indexOfFirstStock + 1}-{indexOfLastStock > filteredStocks.length ? filteredStocks.length : indexOfLastStock}
        </span> of <span className="font-semibold text-gray-900 dark:text-white">{filteredStocks.length}</span>
      </span>
    </div>
    <div className='grid col-span-2'>
      <ul className="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
        ...
        {Array.from({ length: Math.ceil(filteredStocks.length / StockPerPage) }, (_, index) => (
          <li key={index}>
