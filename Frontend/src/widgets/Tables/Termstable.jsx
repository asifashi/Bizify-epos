...
const Termstable = ({settermsId,setEdit,ok, okModal}) => {
  
  const [termss, settermss] = useState([]);
  const [filteredtermss, setFilteredtermss] = useState([]);
  ...
  const fetchtermss = () => {
    ...
    axios
      ...
      .then((response) => {
        settermss(response.data);
        setFilteredtermss(response.data);
        setLoading(false);
      })
      .catch((error) => {
  ...
  const handleRemoveterms = (id) => {
    setOpenModal(true);

    setConfirmationCallback(() => () => {
      axios
        .delete(`http://localhost:8081/api/terms/${id}`)
        .then((response) => {
          ...
          fetchtermss(); // Fetch termss again after deletion
        })
        .catch((error) => {
  ...
  const handleSearchterms = (e) => {
    ...
    const filteredData = termss.filter(
      (terms) => terms.Name.toLowerCase().includes(searchValue) 
    );
    setFilteredtermss(filteredData);
    setCurrentPage(1);
  };

  const indexOfLastterms = currentPage * itemsPerPage;
  const indexOfFirstterms = indexOfLastterms - itemsPerPage;
  const currenttermss = filteredtermss.slice(indexOfFirstterms, indexOfLastterms);
        ...
        <div className='relative bg-gray-100 rounded-lg shadow dark:bg-gray-700'>
          {ok && (
            <Alert color='success' icon={HiInformationCircle}>
              <span className='font-medium'>Info alert!</span> Terms created successfully
            </Alert>
          )}
          {okModal && (
            <Alert color='success' icon={HiInformationCircle}>
              <span className='font-medium'>Info alert!</span> Terms updated successfully
            </Alert>
          )}
          {showDeleteAlert && (
            <Alert color='success' icon={HiInformationCircle}>
              <span className='font-medium'>Info alert!</span> Terms deleted successfully
            </Alert>
          )}
            ...
            <tbody>
              {currenttermss.map((terms, index) => (
                ...
                >
                  ...
                  <td className='px-4  py-2'>{terms.Terms.slice(0, 30)}....<EyeIcon className='cursor-pointer w-4 h-4'/></td>
                  <td className='flex items-center px-4 py-3 space-x-3'>
                    <PencilSquareIcon
                      onClick={(e) =>handleEditterms(terms.id)}
                      className='font-medium w-4 h-4 text-blue-600 hover:cursor-pointer dark:text-blue-500 hover:underline'
                    />
                    <TrashIcon
                      onClick={(e) =>handleRemoveterms(terms.id)}
          ...
          <nav className='flex bg-blue-gray-50 p-1 items-center' aria-label='Table navigation'>
            <div className='grid col-span-2'>
              <ul className='inline-flex -space-x-px rtl:space-x-reverse text-sm h-8'>
                ...
                {Array.from(
                  { length: Math.ceil(filteredtermss.length / itemsPerPage) },
                  (_, index) => (
                    <li key={index}>
                      <a
                        href='#'
