...
const Brandtable = ({setbrandId,setEdit,ok,okModal}) => {
...
const toggleModal = () => {
  ...
  const fetchBrands = () => {
    setLoading(true);
    axios
      .get('http://localhost:8081/api/getBrands')
      .then((response) => {
  ...
  const handleRemoveBrand = (id) => {
    ...
    setConfirmationCallback(() => () => {
      axios
        .delete(`http://localhost:8081/api/brand/${id}`)
        .then((response) => {
          ...
          fetchBrands(); // Fetch brands again after deletion
        })
        .catch((error) => {
  ...
  const handleSearchBrand = (e) => {
    ...
    const filteredData = brandname.filter((brand) =>
        ...
        <div className='relative bg-white rounded-lg shadow dark:bg-gray-700'>
          {ok && (
            <Alert color='success' icon={HiInformationCircle}>
              <span className='font-medium'>Info alert!</span> Brand created successfully
            </Alert>
          )}
          {okModal && (
            <Alert color='success' icon={HiInformationCircle}>
              <span className='font-medium'>Info alert!</span> Brand updated successfully
            </Alert>
          )}
          {showDeleteAlert && (
            <Alert color='success' icon={HiInformationCircle}>
              <span className='font-medium'>Info alert!</span> Brand deleted successfully
            </Alert>
          )}
              ...
              {currentBrands.map((brand, index) => (
                ...
                >
                  ...
                  <td className='flex items-center px-4 py-3 space-x-3'>
                    ...
                    />
                    <TrashIcon
                      onClick={(e) => handleRemoveBrand(brand.id)}
