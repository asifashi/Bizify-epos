...
const DeliverTable = ({setdeliverId,setEdit,setSelecteddeliver,setView,okModaldeliver,okModal}) => {
  const [delivery,setDelivery]=useState([])
  const [filteredDeliver, setFilteredDeliver] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [DeliverPerPage] = useState(10);
  ...
  const  fetchDeliver = () =>{
    setLoading(true); 
    axios.get('http://localhost:8081/api/getdeliver', {
      headers: {
        'access-token': localStorage.getItem('token') || '',
      },})
    .then(response => {
      setDelivery(response.data);
      setFilteredDeliver(response.data)
    })
    .catch(error => {
  ...
  const handleRemove = (VoucherNo) => {
    ...
    axios.delete(`http://localhost:8081/api/remove/${VoucherNo}`)
      .then(response => {
  ...
  const handleEdit = (VoucherNo)=>{
      ...
      const handleSearch = (e) => {
        ...
        if (delivery && delivery.length > 0) {
          const filteredData = delivery.filter((deliver) => {
            const propertyValue = deliver[selectedSearchOption];
          ...
          setFilteredDeliver(filteredData);
          setCurrentPage(1);
        }
      };
      const indexOfLastDeliver = currentPage * DeliverPerPage;
      const indexOfFirstDeliver = indexOfLastDeliver - DeliverPerPage;
      const currentDeliver = filteredDeliver.slice(indexOfFirstDeliver, indexOfLastDeliver);
      const paginatedDeliver = currentDeliver.slice(0, DeliverPerPage);
  ...
  <span className="font-medium">Info alert!</span> Delivery note deleted successfully
</Alert>
)}

{okModaldeliver && (
  <Alert color="success" icon={HiInformationCircle}>
    <span className="font-medium">Info alert!</span>  Delivery note created successfully
  </Alert>
)}
{okModal && (
  <Alert color="success" icon={HiInformationCircle}>
    <span className="font-medium">Info alert!</span>  Delivery note updated successfully
  </Alert>
)}
  ...
  <div className="text-center text-gray-500 dark:text-gray-400 mt-4">
    No search results found.
           ...
           <table className="w-full text-sm text-left text-gray-500 dark:text-gray-600">
              ...
              {Array.isArray(paginatedDeliver) && paginatedDeliver.map(deliver => (
             
                 <tr key={deliver.V_ID} className="bg-white border-b text-xs dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td  onClick={(e) => handleAccounsClick(deliver.VoucherNo)} className="px-4 py-3 cursor-pointer font-medium text-gray-900 whitespace-nowrap dark:text-white">
                  
                     {deliver.CustomerName}
                  
                    </td>
                    <td className="px-4 py-3">
                     {deliver.GrossAmount.toFixed(2)}
                    </td>
                    <td className="px-4 py-3">
                    {new Date(deliver.VoucherDate).toLocaleDateString()}
                    </td>
                    <td className="px-4 py-3">
                    {new Date(deliver.DueDate).toLocaleDateString()}
                    </td>
                    <td className="px-4 py-3 text-yellow-500">
                      {deliver.status}
                    </td>
                    <td className="flex items-center px-4 py-3 space-x-3">
                    <EyeIcon onClick={(e) => handleInvoiceClick(deliver.VoucherNo)} className='w-4 cursor-pointer h-4'/>
                    <PencilSquareIcon onClick={(e)=>handleEdit(deliver.VoucherNo)} className="w-4 h-4 text-blue-600 hover:cursor-pointer dark:text-blue-500 hover:underline"/>
                  <TrashIcon onClick={() => handleRemove(deliver.VoucherNo)} className="font-medium w-4 h-4 hover:cursor-pointer text-red-600 dark:text-red-500 hover:underline"/>
                </td>
    ...
    <div className='grid col-span-2'>
      <span className="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full ">
        Showing <span className="font-semibold text-gray-900 dark:text-white">
          {indexOfFirstDeliver + 1}-{indexOfLastDeliver > filteredDeliver.length ? filteredDeliver.length : indexOfLastDeliver}
        </span> of <span className="font-semibold text-gray-900 dark:text-white">{filteredDeliver.length}</span>
      </span>
    </div>
    <div className='grid col-span-2'>
      <ul className="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
        ...
        {Array.from({ length: Math.ceil(filteredDeliver.length / DeliverPerPage) }, (_, index) => (
          <li key={index}>
