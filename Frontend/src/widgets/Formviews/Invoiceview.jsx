...
const Invoiceview = ({setView,setEdit,setinvoiceId,VoucherNo}) => {
    let subtotal=0;
    ...
    useEffect(() => {
      ...
      const fetchInvoices = () => {
        setLoading(true)
        VoucherNo=VoucherNo
        axios.get(`http://localhost:8081/api/getFormData/${VoucherNo}`)
          .then(response => {
  ...
  return (
    <div className='w-3/4'>
     ...
     <div className='grid grid-col bg-gradient-to-br from-blue-gray-700 to-blue-gray-800 mt-1 rounded-xl shadow-xl h-10'>
        <span className='text-white text-lg p-2'><span className='pl-2 text-white pr-1 text-xl'>Invoice </span>
        <span><PencilSquareIcon onClick={(e)=>handleEdit(invoices && invoices.VoucherNo)}  className='w-5 h-5 cursor-pointer hover:bg-gray-300 flex ml-auto mr-7 justify-center items-center -mt-6'/>
        <button
              type="button"
              onClick={toggleModal}
        ...
        <div className="w-full  max-h-full">
          ...
          <div className="flex items-center justify-center h-64">
           ...
           {!loading && ( <div className="px-3 py-3 text-sm max-w-full">
            ...
            <span className='p-4 text-3xl font-semibold'>Invoice #{invoices && invoices.VoucherNo}</span> 
            <label className='font-semibold bg-white pt-5 pl-32 font-sans'>Date :<label className='px-3 font-normal'>{ invoices && new Date (invoices.VoucherDate).toLocaleDateString()}</label></label>
            </div>
             ...
             <p className='pl-5 pb-4 italic'>{invoices && invoices.CustomerDetail}</p>
              ...
              <tbody>
              {invoices && invoices.items && invoices.items.map((invoice,index) => {
                const product=parseFloat(invoice.Sold_Qty || 0) * parseFloat(invoice.UnitPrice || 0 ) 
                subtotal += product
                return(
